...
class CustomerStore {
	...
	getProducts = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getproducts(getId.distributor_ID || getIdR.distributor_ID).then((resp) => {
	...
	addStockOut = () => {
		...
		this.api.addstockout(this.stockout, dis[0]._id).then((resp) => {
			this.listofProducts = resp.data;
		});
	};
	...
	editCart = () => {
		...
		this.api.editcart(this.cart, cartz[0]._id, getuserId.account_ID).then((resp) => {
			this.listOfCart = resp.data;
		});
	};
	...
	addNotif = () => {
		return new Promise((resolve, reject) => {
			this.api.addnotif(this.notif).then((resp) => {
	...
	getNotif = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getnotif(getId.distributor_ID || getId.account_ID).then((resp) => {
				this.listOfNotif = resp.data;
	...
	addFavorite = () => {
		return new Promise((resolve, reject) => {
			this.api.addfavorite(this.favorite).then((resp) => {
	...
	deleteFavorite = () => {
		...
		this.api.deletefavorite(this.favorite, fave[0]._id, getuserId.account_ID).then((resp) => {
			this.listOfFavorite = resp.data;
		});
	};
	addAccount = () => {
	...
	addMembership = () => {
		return new Promise((resolve, reject) => {
			this.api.addmembership(this.membership).then((resp) => {
	...
	getMembership = () => {
		return new Promise((resolve, reject) => {
			let getDistId = JSON.parse(sessionStorage.getItem('userData'));
			this.api.getmembership(getDistId.account_ID).then((resp) => {
				this.listOfMembership = resp.data;
