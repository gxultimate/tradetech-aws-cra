...
class StartingStore {
	...
	getProductsR = () => {
		return new Promise((resolve, reject) => {
			this.api.getproducts(this.product.distributor_ID).then((resp) => {
				this.listofProducts = resp.data;
	...
	getProductImg = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getproductImg(getId.distributor_ID).then((resp) => {
	...
	getProductImgR = () => {
		return new Promise((resolve, reject) => {
			this.api.getproductImg(this.product.distributor_ID).then((resp) => {
				this.listofProductImg = resp.data;
	...
	createToken = (data) => {
		return new Promise((resolve, reject) => {
			this.api.logintoken(data).then((resp) => {
				if (resp.data !== false) {
					resolve(resp.data);
				} else {
					resolve(false);
	...
	loginAccount = () => {
		return new Promise((resolve, reject) => {
			this.api.loginaccount(this.account).then((resp) => {
				...
				let access = resp.data.account_accessType
					? resp.data.account_accessType
					: resp.data.distributor_accessType;
	...
	addProduct = () => {
		return new Promise((resolve, reject) => {
			this.api.addproduct(this.product).then((resp) => {
	...
	editProduct = () => {
		...
		this.api.editproduct(this.product, prod[0]._id).then((resp) => {
			this.listofProducts = resp.data;
		});
	};
	...
	addDistributor = () => {
		this.api.adddistributor(this.distributor).then((resp) => {
	...
	editDistributor = () => {
		...
		this.api.editdistributor(this.distributor, dis[0]._id).then((resp) => {
			this.listOfDistributors = resp.data;
		});
	};
	...
	archiveDistributor = () => {
		...
		this.api.archivedistributor(this.distributor, dis[0]._id).then((resp) => {
			this.listOfDistributors = resp.data;
		});
	};
	...
	addStock = () => {
		...
		this.api.addstock(this.stock, dis[0]._id).then((resp) => {
			this.listofProducts = resp.data;
		});
	};
	...
	getStock = () => {
		return new Promise((resolve, reject) => {
			let getDistId = JSON.parse(sessionStorage.getItem('userData'));
			this.api.getstock(getDistId.distributor_ID).then((resp) => {
	...
	getCart = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getcart(getuserId.account_ID).then((resp) => {
	...
	getCartD = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getcart(getuserId.distributor_ID).then((resp) => {
				this.listOfCart = resp.data;
	...
	editCart = () => {
		...
		this.api.editcart(this.cart, cartz[0]._id, getuserId.account_ID, getuserId.distributor_ID).then((resp) => {
	...
	deleteCart = () => {
		...
		this.api.deletecart(this.cart, cartz[0]._id, getuserId.account_ID).then((resp) => {
			this.listOfCart = resp.data;
		});
	};
	...
	getOrderD = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getorder(this.order.account_ID ? this.order.account_ID : getuserId.distributor_ID).then((resp) => {
				this.listOfOrder = resp.data;
	...
	assignOrder = () => {
		...
		this.api.assignorder(this.order, dis[0]._id).then((resp) => {
			this.listOfOrder = resp.data;
		});
	};

	addToken = () => {
		this.api.addtoken(this.token).then((resp) => {
	...
	deletetoken = () => {
		...
		this.api.deletetoken(this.token, mytoken[0]._id).then((resp) => {
			this.listOfToken = resp.data;
		});
	};
	...
	getToken = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.gettoken(getuserId.distributor_ID).then((resp) => {
				this.listOfToken = resp.data;
	...
	accessDistributor = () => {
		return new Promise((resolve, reject) => {
			this.api.accessdistributor(this.token.access_Token).then((resp) => {
	...
	addReport = () => {
		return new Promise((resolve, reject) => {
			this.api.addreport(this.report).then((resp) => {
	...
	addcLogs = () => {
		return new Promise((resolve, reject) => {
			this.api.addclogs(this.cLogs).then((resp) => {
	...
	archiveAccount = () => {
		...
		this.api.archiveaccount(this.account, user[0]._id).then((resp) => {
			this.listOfUsers = resp.data;
		});
	};
	getNotif = () => {
		return new Promise((resolve, reject) => {
			...
			this.api.getnotif(getId.distributor_ID).then((resp) => {
	...
	addMessage = () => {
		this.api.addmessage(this.message).then((resp) => {
			this.listOfMessage = resp.data;
		});
	};
	getMessage = () => {
		return new Promise((resolve, reject) => {
			this.api.getmessage().then((resp) => {
				this.listOfMessage = resp.data;
