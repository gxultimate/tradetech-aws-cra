...
let filpacking =listOfOrder.filter(order => order.orderStatus === 'Transfer' && order.distributor_ID === getDis.distributor_ID)
...
<Typography  style={{textAlign:"left"}}>In Charge : <span style={{fontWeight:"bold"}}> {listOfUsers.filter(accs => accs.account_ID === orders.packer_ID).map((account)=> {return `${account.account_fName} ${account.account_mName} ${account.account_lName}`  } ) }</span></Typography>
 ...
 function PackGrid() {
  ...
  return (
    ...
    <Grid item xs={9} sm={9}>
      <Typography variant="h5"  style={{color:"#208769",float:"left",marginLeft:"5px"}}>Orders on dispatcher</Typography>
    </Grid>
