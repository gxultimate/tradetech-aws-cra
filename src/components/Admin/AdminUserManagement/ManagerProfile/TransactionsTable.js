...
let {employeeStore:{listOfOrder,order}}=this.props;

function createData(orderInfo,name, calories, fat, carbs, protein) {
  return { orderInfo,name, calories, fat, carbs, protein };
}


let filorder =listOfOrder.filter(order =>{

  if(  order.packer_ID === this.props.account_ID  ){
    return order
  }else if( order.dispatcher_ID === this.props.account_ID  ){
    return order
  }
...
let rows =filorder.map(order => {
 ...
 order,order.orderDate,order.orderID,order.orderStatus,order.account_ID,order.paymentStatus
...
export default inject('employeeStore')(observer(TransactionsTable));