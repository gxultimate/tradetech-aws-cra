...
import moment from 'moment'
 ...
 class InvoiceD extends React.Component {
  componentDidMount(){
    let {reportStore:{getAccounts,getOrder }}=this.props;
    getAccounts();
    getOrder();
  }
...
function createData(name, orddate, due, cust,mname,lname, amtdue, bal, ordstat) {
  return { name, orddate, due, cust,mname,lname, amtdue, bal, ordstat };
}

let rows =listOfOrder.map(order => {
  return (createData(
  order.orderID,order.orderDate,order.orderDueDate,`${listOfUsers.filter(user=> user.account_ID === order.account_ID).map(name =>{
    return (
      `${name.account_fName}`
    )
  })}`,`${listOfUsers.filter(user=> user.account_ID === order.account_ID).map(name =>{
    return (
      `${name.account_mName}`
    )
  })}`,`${listOfUsers.filter(user=> user.account_ID === order.account_ID).map(name =>{
  ...
  })}`,order.orderTotalAmount,order.orderCustomerBalance,order.orderStatus
  ))
})
...
const headCells = [
  { id: 'name', numeric: false, disablePadding: false, label: 'Invoice #' },
  { id: 'orddate', numeric: true, disablePadding: false, label: 'Order Date' },
  { id: 'due', numeric: true, disablePadding: false, label: 'Due Date' },
  { id: 'cust', numeric: true, disablePadding: false, label: 'Customer' },
  { id: 'amtdue', numeric: true, disablePadding: false, label: 'Amount Due' },
  { id: 'bal', numeric: true, disablePadding: false, label: 'Balance' },
  { id: 'ordstat', numeric: true, disablePadding: false, label: 'Order Status' },
];
...
function InvoiceDTableHead(props) {
...
function InvoiceDTable() {
  ...
  const [orderBy, setOrderBy] = React.useState('orddate');
...
export default inject('reportStore')(observer(InvoiceD))
