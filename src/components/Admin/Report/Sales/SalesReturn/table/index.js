...
class SReturn extends React.Component {
...
function createData(name, ref, cust,mname,lname, due, rdate,stat,ramt,bal,mop) {
  return { name, ref, cust,mname,lname, due, rdate,stat,ramt,bal,mop };
}



let rows = listOfOrder.filter(ord=> ord.orderStatus === 'Failed').map(order =>{
  return (createData(
  order.orderDate,order.orderID,`${listOfUsers.filter(name => name.account_ID === order.account_ID).map(fname =>{
    return `${fname.account_fName}`
  })}`,`${listOfUsers.filter(name => name.account_ID === order.account_ID).map(fname =>{
    return `${fname.account_mName}`
  })}`,`${listOfUsers.filter(name => name.account_ID === order.account_ID).map(fname =>{
    return `${fname.account_lName}`
  })}`,`${order.orderTotalAmount}`,
  order.orderReturnDate,order.orderStatus,'null',order.orderCustomerBalance
  ,order.modeOfPayment
...
function SReturnTableHead(props) {
...
export default inject('reportStore')(observer(SReturn))
