...
class ArchivedTBL extends React.Component {
  ...
  render() { 
    ...
    let myID = JSON.parse(sessionStorage.getItem('userData'));
        let listOfcustomer = listOfUsers.filter(user => user.account_accessType === "customer" && user.distributor_ID === myID.distributor_ID && user.account_status === 'archived');
...
function ArchivedTableHead(props) {
  ...
  return (
    ...
    <Dialog
      ...
      <Grid container xs={12}>
        {/* <ArchivedTableToolbar numSelected={selected.length} /> */}
...
export default withRouter(inject('crmStore')(observer(ArchivedTBL)));