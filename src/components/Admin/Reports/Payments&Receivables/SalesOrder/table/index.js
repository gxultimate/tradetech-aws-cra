...
componentDidMount(){
  let {reportStore:{getOrder,getAccounts,getDistributors}}=this.props;
  getOrder();
  getAccounts();
  getDistributors()
}
  ...
  render() { 
let {reportStore:{listOfOrder,listOfUsers,order}}=this.props;
              ...
              {stableSort(rows, getComparator(orders, orderBy))
                ...
                .map((row, index) => {
                  ...
                  if(filter.length !== 0 || edate._isValid === true){
                    if(  mydate.isBetween(sdate,edate)  || strmydate == strsdate || strmydate == stredate || row.ref.startsWith(filter) || 
...
export default inject('reportStore')(observer(SummaryTBL));