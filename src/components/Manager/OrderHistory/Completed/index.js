...
function createData(ref, date, dateCompleted, info, protein) {
  return { ref, date, dateCompleted, info, protein };
}
...
let filterComplete =listOfOrder.filter(order => {
   if(order.orderStatus === 'Completed' &&  myId.staff_Role === 'Dispatcher Manager' && order.distributor_ID ===  myId.distributor_ID || order.orderStatus != 'Failed' && order.orderStatus != 'Pending' && myId.staff_Role === 'Packer Manager' && order.distributor_ID ===  myId.distributor_ID ) {
       return order
   }
...
const headCells = [
  ...
  { id: 'info', numeric: true, disablePadding: false, label: 'More Info' },
...
const Transition = React.forwardRef(function Transition(props, ref) {
 ...
 function Completed() {
  ...
  let rows = filterComplete.map(orderss => {
    return(createData(
        orderss.orderID,orderss.orderDate,orderss.orderDateCompleted,<div><ThemeProvider theme={theme}> <Button size='small' variant="outlined" color="primary"  
...
export default inject('managerStore')(observer(CompletedTable));
