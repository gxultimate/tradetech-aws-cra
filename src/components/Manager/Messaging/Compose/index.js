...
import React, { Component } from 'react'
 ...
 class ComposeMessage extends Component {
  componentDidMount(){
    let{managerStore:{getAccounts,getMessage,getDistributors}}=this.props;
     getDistributors()
    getMessage()
}
    render() {
let {managerStore:{listOfDistributors,message,addMessage}}=this.props;

let myAccount = JSON.parse(sessionStorage.getItem('userData'))
let distAccount = listOfDistributors.filter(acc=> acc.distributor_ID === myAccount.distributor_ID).map(dst=> {return dst.distributor_warehouseName})
let distAccountID = listOfDistributors.filter(acc=> acc.distributor_ID === myAccount.distributor_ID).map(dst=> {return dst.distributor_ID})
...
function ComposeDialogs() {
  ...
  message.setProperty('recipient_Name',distAccount[0])
  message.setProperty('recipient_ID',distAccountID[0])
...
export default inject('managerStore')(observer(ComposeMessage))